 ...
 * }, 'room-id');
 */
async function getDeviceByRoomId(options, roomId) {
  ...
  const devicesPlain = devices.map((device) => {
    ...
    rawDevice.features.forEach((feature) => {
      ...
      const lastValueIsTooOld = lastValueInTimestamp < tooOldTimestamp;
      feature.last_value_is_too_old = lastValueIsTooOld;
    });
    return rawDevice;
  });
